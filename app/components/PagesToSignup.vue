<template>
  <div class="dash-card">
    <h2 class="dash-title">Pages Leading to Signup</h2>
    <p class="dash-help">Which pages visitors come from before reaching the Signup page. Shows what content drives people to sign up.</p>
    <LoadingSpinner v-if="pending" />
    <ErrorAlert v-else-if="error" :message="error.message" @retry="refresh" />
    <EmptyState v-else-if="!rows.length" title="No paths to signup yet" description="This will show once visitors start navigating between pages and reaching the signup page." />
    <div v-else>
      <div v-for="(r, i) in rows" :key="i" class="flex items-center gap-3" :style="{ padding: '12px 0', borderBottom: i < rows.length - 1 ? '1px solid var(--dash-border-row)' : 'none' }">
        ...
        <span style="font-size: 14px; color: var(--dash-text-ghost);">→ /signup</span>
...
const { data, pending, error, refresh } = useFetch(() => `/api/posthog/pages-to-signup?days=${period.value}`, { watch: [period, refreshKey] })
