    ...
    <div v-else class="grid grid-cols-1 sm:grid-cols-2 gap-6">
      <div v-for="(r, i) in rows" :key="i" style="background: var(--dash-bg-inset); border: 1px solid var(--dash-border-card); border-radius: 16px; padding: 24px;">
        ...
        <div class="flex items-center justify-between" style="padding: 10px 0; border-bottom: 1px solid var(--dash-border-row);">
          ...
          <span style="font-size: 16px; font-weight: 700; color: var(--dash-text-primary);" class="tabular-nums">{{ r.forms }} <span style="font-size: 14px; font-weight: 400; color: var(--dash-text-ghost);">from {{ r.submitters }} people</span></span>
        </div>
        <div class="flex items-center justify-between" style="padding: 10px 0; border-bottom: 1px solid var(--dash-border-row);">
...
const rows = computed(() => ((data.value as any)?.results ?? []).map(([device, pageviews, forms, submitters, signups, phoneCalls, users]: [string, number, number, number, number, number, number]) => ({
  device, pageviews, forms, submitters, signups, phoneCalls, users,
