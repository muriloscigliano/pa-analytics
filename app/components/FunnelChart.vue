      ...
      <div v-for="(step, i) in steps" :key="i">
        <div class="flex items-center justify-between mb-2">
          <div class="flex items-center gap-2">
            ...
            <span v-if="step.pending" style="font-size: 12px; color: var(--dash-text-ghost); font-style: italic;">(not yet instrumented)</span>
          </div>
        ...
        <div v-if="i < steps.length - 1 && !step.pending && !steps[i + 1].pending" class="dropoff-label">
...
const PENDING_EVENTS = new Set(['signup_completed_server'])
const steps = computed(() => {
  if (!data.value || !Array.isArray(data.value)) return []
  const raw = data.value as Array<{ custom_name: string; count: number; name?: string }>
  const total = raw[0]?.count || 1
  return raw.map((step, i) => ({
    name: step.custom_name, count: step.count,
    pending: PENDING_EVENTS.has(step.name || ''),
    rate: Math.round((step.count / total) * 100),
    dropoff: i < raw.length - 1 ? Math.round(((step.count - raw[i + 1].count) / (step.count || 1)) * 100) : 0,
  }))
})
</script>
